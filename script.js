...
function closeMinEditBox() {
    ...
    function hidebox(e) {
        // 'e' is a pointer event object that keeps the complete info. of the event that is performed, in our case the 'click' event. It is passed as a parameter by JS to the function that is called

        // console.log(e.target)   //'target' is a key inside 'e' object that contains the HTML of the element on which the event is performed

        let box = e.target.closest(".box") // 'closest' is a predefined function that is used to select the HTML of the element whose class is provided that is nearest to the target element. P.S. the target element should be the child of the element whose class is provided to closest function
    ...
    function boxedit(e) {
        ...
        popupWrap.style.justifyContent = 'center'
        let add = document.querySelector(".add")
        add.innerHTML = `<button class="editbtn">Save</button>`
